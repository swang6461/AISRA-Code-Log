/*
UPDATED: Short-travel stability fix only. No other logic changed.
* Added minimum ramp floor so short moves do not accelerate/decelerate too abruptly
* Added ramp clamp so ramp never exceeds half of total move distance
* Lowered ramp start/end speed from 100 to 30 for smoother short moves
* Tightened decel condition from '>' logic to '>=' boundary-safe logic
...
void moveWithAcceleration(float outputAngleDeg, long maxStepsPerSec, bool cw) {
 ...
 for (long i = 0; i < totalSteps; i++) {
   ...
   if (i < rampSteps) {
     currentSpeed = map(i, 0, rampSteps, 30, maxStepsPerSec);
   } else if (i >= (totalSteps - rampSteps)) {
     currentSpeed = map(i, totalSteps - rampSteps, totalSteps - 1, maxStepsPerSec, 30);
   } else {
